...
| `src/config.ts` | Конфиг: `process.env` > `.env` (свой парсер `parseDotEnv`) > `glw.config.json`; `loadConfig(override?, requireProject)` |
...
| `src/cache.ts` | Кеш путей проектов `~/.glw/projects.json` для shell-автокомплита (best-effort, ошибки записи молча глотаются) |
...
- Меняешь поверхность команд (флаги/команды) — синхронно обнови **три** места: `--help`-тексты, `README.md`, навык `.claude/skills/glw/`.
...
- `.claude/skills/glw/SKILL.md` — навык, обучающий агентов пользоваться `glw` (preflight, bulk-протокол, маппинг намерений на команды).
